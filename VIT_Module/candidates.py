...
class CandidatesPage(QWidget):
    ...
    def get_candidates(self):
        try:
            ...
            # Activate the Assign Mentor context menu for this case
            myf.enable_context_menu(self.form_candidates, self.show_context_menu_assign_mentor)
            # self.disable_cell_entered_signal()

            self.headers = ['Zaman damgası', 'Basvuru Donemi', 'Basvuran Ad', 'Basvuran Soyad', 'Basvuran Mail',
                            'Basvuran IT sektör bilgisi', 'Basvuran yoğunluk', 'Düşünceler', 'Yorumlar',
            ...
            applicants = myf.execute_read_query(cnf.open_conn(), q1, (myf.last_period(), 1))

            # Rebuilds the list based on the data type of the cells.
            applicants = myf.remake_it_with_types(applicants)
    ...
    def show_open_appointments(self):
        try:
            myf.disable_context_menu(self.form_candidates, self.show_context_menu_assign_mentor)  # Disable Assign Mentor context menu
            cnf = Config()
    ...
    def assign_mentor(self, event_id):
        # BU FONKSIYONDA YAKLASIM DEGISIMI GEREKIOR
        try:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                basvuran_id = self.form_candidates.tableWidget.item(current_row, 22).text()
                ...
                q2 = ("UPDATE " + cnf.evaluationTable + " SET " + cnf.evaluationTableFieldNames[13] + " = 2 WHERE "
